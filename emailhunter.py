...
class Module(BaseModule):
    ...
    def get_name(self, email, title):
        ...
        if len(parts) == 2:
            answer.update(
                {
                    "first_name": up(parts[0]),
                    "middle_name": None,
                    "last_name": up(parts[1]),
                    "email": email,
                    "title": title,
                }
            )
            ...
            answer.update(
                {
                    "first_name": up(parts[0]),
                    "middle_name": up(parts[1]),
                    "last_name": up(parts[2]),
                    "email": email,
                    "title": title,
                }
            )
        ...
        for domain in domains:
            ...
            if resp.status_code == 200:
                ...
                for email in resp.json()["emails"]:
                    email_user_data = self.get_name(
                        email=email["value"], title="EmailHunter contact"
                    )
                    self.insert_contacts(**email_user_data)
