...
class Module(BaseModule):
    ...
    def module_run(self, ips):
        ...
        for ip in ips:
            ...
            urls = ["http://whois.arin.net/ui/query.do"]
            for url in urls:
                self.verbose(f"URL: {url}")
                resp = self.request("POST", url, headers=headers, data=payload).json()

                if (
                    self.get_orgRef(resp) == "APNIC"
                    or self.get_orgRef(resp) == "RIPE"
                    or self.get_orgRef(resp) == "LACNIC"
                    or self.get_orgRef(resp) == "AFRINIC"
                ):
                    self.output(
                        "Error, %s is %s."
                        % (ip, resp["ns4:pft"]["net"]["orgRef"]["@name"])
                    )
                    continue
                ...
                if type(netblocks) == dict:  # single net block
                    ...
                    self.output(
                        "%s is in netblock %s and belongs to %s" % (ip, nb, org)
                    )
                ...
                self.query(
                    "UPDATE hosts SET netblock=? WHERE ip_address=?", (host_nb, ip)
                )
